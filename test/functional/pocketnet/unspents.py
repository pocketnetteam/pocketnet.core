...
class UnspentsTest(PocketcoinTestFramework):
    ...
    def run_test(self):
        ...
        # ---------------------------------------------------------------------------------
        self.log.info("Check first unspent")
        assert_equal(len(node.public().txunspent([accounts[0].Address])), 50)
        pubGenTx(accounts[0], ContentPostPayload())
        assert_equal(len(node.public().txunspent([accounts[0].Address])), 49)
        node.stakeblock(1)
        assert_equal(len(node.public().txunspent([accounts[0].Address])), 50)
