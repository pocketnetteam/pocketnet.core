    ...
    for (int i = 0; i < 10; i++) {
        threads.emplace_back([&]() {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                auto pocketBlockRef = std::make_shared<PocketBlock>(pocketBlock);
                Assert(m_node.chainman)->ProcessNewBlock(state, Params(), block, pocketBlockRef, true, &ignored);
            }
            ...
            for (auto block : blocks) {
                if (block->vtx.size() == 1) {
                    ...
                    auto pocketBlockRef = std::make_shared<PocketBlock>(pocketBlock);
                    bool processed = Assert(m_node.chainman)->ProcessNewBlock(state, Params(), block, pocketBlockRef, true, &ignored);
                    assert(processed);
                }
            }
        });
