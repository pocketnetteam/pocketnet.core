 ...
 *  Pocketcoin because since the activation of BIP66, signatures are verified to be
...
void ConvertToDER(std::vector<unsigned char>& signature, std::vector<unsigned char>& der_signature) {
    ...
    //Length of rValue
    if (rValue.at(0) >= 0x80) {
        der_signature.push_back(rValue.size() + 1);
    }
    else {
        der_signature.push_back(rValue.size());
    }
    ...
    for (unsigned char b : rValue) {
        if (b >= 0x80 && c == 0) {
            der_signature.push_back(0);
        }
        der_signature.push_back(b);
        c++;
    }
    ...
    //Length of sValue
    if (sValue.at(0) >= 0x80) {
        der_signature.push_back(sValue.size() + 1);
    }
    else {
        der_signature.push_back(sValue.size());
    }
    ...
    int len = der_signature.size() - 1;
    der_signature.insert(der_signature.begin() + 1, len);
}