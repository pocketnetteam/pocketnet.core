...
bool GenerateOuts(CAmount nCredit, std::vector<CTxOut>& results, CAmount& totalAmount, std::vector<std::string> winners, opcodetype op_code_type, int height, std::vector<opcodetype>& winner_types)
{
    if (winners.size() > 0 && winners.size() <= 25) {
        ...
        if (height >= Params().GetConsensus().checkpoint_0_19_3) {
            // Referrer program 5 - 100%; 4.5 - nodes; 0.5 - all for lottery;
            // 1.0 - posts (20%); 1.0 - referrer over posts (20%); 0.5 - comment (10%); 0.5 - referrer over comment (10%);
            if (op_code_type == OP_WINNER_POST) ratingReward = nCredit * 0.04;
            if (op_code_type == OP_WINNER_POST_REFERRAL) ratingReward = nCredit * 0.04;
            if (op_code_type == OP_WINNER_COMMENT) ratingReward = nCredit * 0.01;
            if (op_code_type == OP_WINNER_COMMENT_REFERRAL) ratingReward = nCredit * 0.01;
