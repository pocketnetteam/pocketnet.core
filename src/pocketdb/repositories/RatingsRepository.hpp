...
namespace PocketDb {
    ...
    public:
        ...
        tuple<bool, int> GetScorePostCount(const string scoreAddress, const string postAddress, int height,
            const CTransactionRef& tx, const std::vector<int> values, int64_t scoresOneToOneDepth)
        {
            ...
            bool tryResult = TryTransactionStep([&](){
                string sql = R"sql(
                    select count(1)
                    from vScorePosts s
                    inner join vPosts p on p.Hash = s.PostTxHash
                    where s.AddressHash = ?
                        and p.AddressHash = ?
                        and s.Height <= ?
                        and s.Time < ?
                ...
                auto postAddressPtr = make_shared<string>(postAddress);
                auto heightPtr = make_shared<int>(height);
                ...
                for (auto i = 0; i < values.size(); i++)
                {
                    ...
                    bindResult &= TryBindStatementInt(stmt, 7 + i, valuePtr);
                }
                if (!bindResult) {
                    FinalizeSqlStatement(*stmt);
                    throw runtime_error(strprintf("%s: can't get score post count (bind)\n", func));
                }
        ...
        tuple<bool, int> GetScoreCommentCount(const string scoreAddress, const string commentAddress, int height,
            const CTransactionRef& tx, const std::vector<int> values, int64_t scoresOneToOneDepth)
        {
            int result = 0;
            auto func = __func__;
            ...
            bool tryResult = TryTransactionStep([&]() {
                string sql = R"sql(
                    select count(1)
                    from vScorePosts s
                    inner join vComments p on p.Hash = s.CommentTxHash
                    where s.AddressHash = ?
                        and p.AddressHash = ?
                        and s.Height <= ?
                        and s.Time < ?
                ...
                auto scoreAddressPtr = make_shared<string>(scoreAddress);
                auto postAddressPtr = make_shared<string>(commentAddress);
                auto heightPtr = make_shared<int>(height);
                auto maxTimePtr = make_shared<int64_t>(tx->nTime);
                auto minTimePtr = make_shared<int64_t>((int64_t)tx->nTime - scoresOneToOneDepth);
                auto hashPtr = make_shared<string>(tx->GetHash().GetHex()); //TODO check
