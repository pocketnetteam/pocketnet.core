        ...
        string sql = R"sql(
            with
                ...
                byTags as (
                    ...
                    join
                        web.Tags tag on
                            tag.Lang = lang.value and
                            tag.value in ( )sql" + join(vector<string>(tags.size(), "?"), ",") + R"sql( )
                    join
                        web.TagsMap tm on
                            tm.TagId = tag.Id
                    group by
                        tm.ContentId
                    having
                        ( 1 or max(case when tag.value in ( 'юмор' ) then 1 else 0 end) = 0 )
                )
            ...
            select
                ...
                ct.Height
            from
                Chain ct indexed by Chain_Uid_Desc_Height_Desc

--             cross join
--                 byTags on
--                     ct.Uid = byTags.ContentId

            cross join
                topContentId on
                    ( 0 or ct.Uid < topContentId.value )
            cross join
                height on
                    ct.Height <= height.value
            cross join
                Transactions t on
                    t.RowId = ct.TxId and
                    t.Type in ( 200,201,202,209,210,211 ) and
            ...
            cross join
                minReputation on
                    ifnull(ur.Value, 0) > minReputation.value
    ...
    std::vector<ShortForm> WebRpcRepository::GetEventsForAddresses(const std::string& address, int64_t heightMax, int64_t heightMin, int64_t blockNumMax, const std::set<ShortTxType>& filters)
    {
        ...
        std::vector<PocketDb::ShortForm> shortResult(result.begin(), result.begin() + std::min(10, (int)result.size()));
        return shortResult;
    }

    std::map<std::string, std::map<ShortTxType, int>> WebRpcRepository::GetNotificationsSummary(int64_t heightMax, int64_t heightMin, const std::vector<std::string>& addresses, const std::set<ShortTxType>& filters)
    {
        const std::map<ShortTxType, string> selects = {
            {
                ShortTxType::Referal, R"sql(
