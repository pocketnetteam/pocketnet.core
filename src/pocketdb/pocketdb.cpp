...
Error PocketDB::RestorePostItem(std::string posttxid, int height) {
    ...
    if (err.ok()) {
        ...
        int sum = 0;
        int cnt = 0;
        GetPostRating(posttxid, sum, cnt, height);
...
void PocketDB::GetPostRating(std::string posttxid, int& sum, int& cnt, int height)
{
...
bool PocketDB::UpdatePostRating(std::string posttxid, int sum, int cnt)
{
...
bool PocketDB::UpdatePostRating(std::string posttxid, int height)
{
    ...
    return UpdatePostRating(posttxid, sum, cnt);
}
