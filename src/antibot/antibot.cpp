...
bool AntiBot::CheckRegistration(std::string _address, std::string _txid, BlockVTX blockVtx) {
    if (!g_pocketdb->Exists(reindexer::Query("UsersView", 0, 1).Where("address", CondEq, _address))) {
        // Maybe registration in this block?
...
bool AntiBot::check_post(UniValue oitm, BlockVTX blockVtx, bool checkMempool, ANTIBOTRESULT &result)
{
    ...
    if (!CheckRegistration(_address, _txid, blockVtx)) {
        result = ANTIBOTRESULT::NotRegistered;
        return false;
    }

    // Compute count of posts for last 24 hours
    int postsCount = g_pocketdb->SelectCount(Query("Posts").Where("address", CondEq, _address).Where("txidEdit", CondEq, "").Where("time", CondGe, oitm["time"].get_int64() - 86400));
    postsCount += g_pocketdb->SelectCount(Query("PostsHistory").Where("address", CondEq, _address).Where("txidEdit", CondEq, "").Where("time", CondGe, oitm["time"].get_int64() - 86400));
    ...
    if (checkMempool) {
        ...
        if (g_pocketdb->Select(reindexer::Query("Mempool").Where("table", CondEq, "Posts").Where("txid_source", CondEq, "").Not().Where("txid", CondEq, _txid), res).ok()) {
            for (auto& m : res) {
                ...
                if (t_itm.FromJSON(t_src).ok()) {
                    if (t_itm["time"].As<int64_t>() <= oitm["time"].get_int64() && t_itm["address"].As<string>() == _address && t_itm["txidEdit"].As<string>() == "") {
                        postsCount += 1;
                    }
                }
            }
    ...
    if (blockVtx.Exists("Posts")) {
        for (auto& mtx : blockVtx.Data["Posts"]) {
            if (mtx["txid"].get_str() != _txid && mtx["address"].get_str() == _address && mtx["time"].get_int64() <= oitm["time"].get_int64() && mtx["txidEdit"].get_str() == "") {
                postsCount += 1;
            }
        }
    }
...
bool AntiBot::check_score(UniValue oitm, BlockVTX blockVtx, bool checkMempool, ANTIBOTRESULT &result)
{
    ...
    if (!g_pocketdb->DB()->Select(
        reindexer::Query("Scores")
        .Where("address", CondEq, _address)
        .Where("time", CondGe, oitm["time"].get_int64() - 86400),
        scoresRes
    ).ok()) {
        result = ANTIBOTRESULT::Failed;
        return false;
    ...
    if (checkMempool) {
        ...
        if (g_pocketdb->Select(reindexer::Query("Mempool").Where("table", CondEq, "Scores").Not().Where("txid", CondEq, _txid), res).ok()) {
            for (auto& m : res) {
                ...
                if (t_itm.FromJSON(t_src).ok()) {
                    if (t_itm["time"].As<int64_t>() <= oitm["time"].get_int64() && t_itm["address"].As<string>() == _address) {
                        scoresCount += 1;
                    }
    ...
    if (blockVtx.Exists("Scores")) {
        for (auto& mtx : blockVtx.Data["Scores"]) {
            if (mtx["txid"].get_str() != _txid && mtx["address"].get_str() == _address && mtx["time"].get_int64() <= oitm["time"].get_int64()) {
                scoresCount += 1;
            }
...
bool AntiBot::check_subscribe(UniValue oitm, BlockVTX blockVtx, bool checkMempool, ANTIBOTRESULT &result)
{
    ...
    if (!CheckRegistration(_address_to, _txid, blockVtx)) {
        result = ANTIBOTRESULT::NotRegistered;
        return false;
    }
    ...
    if (checkMempool) {
        ...
        if (g_pocketdb->Select(reindexer::Query("Mempool").Where("table", CondEq, "Subscribes").Not().Where("txid", CondEq, _txid), res).ok()) {
            for (auto& m : res) {
                ...
                if (t_itm.FromJSON(t_src).ok()) {
                    if (t_itm["time"].As<int64_t>() <= oitm["time"].get_int64() && t_itm["address"].As<string>() == _address && t_itm["address_to"].As<string>() == _address_to) {
