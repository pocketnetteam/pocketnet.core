...
void webrtc::signaling::SignalingServer::Init(const std::string &ep, const short &port)
{
    ...
    endpoint.on_open = [proc = m_processor](std::shared_ptr<Connection> connection) {
        proc->OnNewConnection(std::move(connection));
    };

    endpoint.on_close = [proc = m_processor](std::shared_ptr<Connection> connection, int, const std::string&) {
    ...
    endpoint.on_message = [proc = m_processor](std::shared_ptr<Connection> connection, std::shared_ptr<Message> in_message) {
