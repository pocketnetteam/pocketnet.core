...
UniValue getrawtransactionwithmessage(const JSONRPCRequest& request)
{
    ...
    if (address_to != "") {
        ...
        err = g_pocketdb->DB()->Select(
            reindexer::Query("Posts" /*, 0, resultCount*/).Where("address", CondSet, addrs).Not().Where("address", CondSet, addrsblock).Where("time", ((resultCount > 0 && resultStart > 0) ? CondLt : CondGt), resultStart).Sort("time", (resultCount > 0 ? true : false)),
            queryRes);
    } else {
        err = g_pocketdb->DB()->Select(
            reindexer::Query("Posts" /*, 0, resultCount*/).Not().Where("address", CondSet, addrsblock).Where("time", ((resultCount > 0 && resultStart > 0) ? CondLt : CondGt), resultStart).Sort("time", (resultCount > 0 ? true : false)),
            queryRes);
    }
...
UniValue gethotposts(const JSONRPCRequest& request)
{
    ...
    g_pocketdb->Select(reindexer::Query("Posts", 0, count * 5).Where("time", CondGt, curTime - depth).Sort("scoreSum", true).Sort("scoreCnt", true), postsRes);
    ...
    for (auto& p : postsRes) {
        ...
        if (postItm["scoreSum"].As<int>() - (postItm["scoreCnt"].As<int>() * 3) > 0) {
