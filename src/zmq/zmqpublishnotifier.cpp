...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    if (i==mapPublishNotifiers.end())
    {
        ...
        LogPrintCategory(BCLog::ZMQ, "zmq: Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
    ...
    {
        LogPrintCategory(BCLog::ZMQ, "zmq: Reusing socket for address %s\n", address);
        LogPrintCategory(BCLog::ZMQ, "zmq: Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrintCategory(BCLog::ZMQ, "zmq: Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish hashblock %s to %s\n", hash.GetHex(), this->address);
    char data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish hashtx %s to %s\n", hash.GetHex(), this->address);
    char data[32];
    for (unsigned int i = 0; i < 32; i++)
        data[31 - i] = hash.begin()[i];
...
{
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish rawblock %s to %s\n", pindex->GetBlockHash().GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish rawtx %s to %s\n", hash.GetHex(), this->address);
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION | RPCSerializationFlags());
    ss << transaction;
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish sequence block connect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish sequence block disconnect %s to %s\n", hash.GetHex(), this->address);
    char data[sizeof(uint256)+1];
    for (unsigned int i = 0; i < sizeof(uint256); i++)
        data[sizeof(uint256) - 1 - i] = hash.begin()[i];
...
{
    uint256 hash = transaction.GetHash();
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish hashtx mempool acceptance %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrintCategory(BCLog::ZMQ, "zmq: Publish hashtx mempool removal %s to %s\n", hash.GetHex(), this->address);
    unsigned char data[sizeof(uint256)+sizeof(mempool_sequence)+1];
    for (unsigned int i = 0; i < sizeof(uint256); i++)
        data[sizeof(uint256) - 1 - i] = hash.begin()[i];
