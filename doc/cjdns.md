# CJDNS support in Pocketnet Core

It is possible to run Pocketnet Core over CJDNS, an encrypted IPv6 network that
...
done outside of Pocketnet Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Pocketnet network and individual nodes.
...
before it will work with your Pocketnet Core node. This is described in steps
...
## Run Pocketnet Core with CJDNS

Once you are connected to the CJDNS network, the following Pocketnet Core
...
When enabled, this option tells Pocketnet Core that it is running in an
...
IPv6 local network. This helps Pocketnet Core perform better address management:
...
CJDNS support was added to Pocketnet Core in version 0.22.5 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `pocketcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/pocketnetteam/pocketnet.core/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Pocketnet Core

There are several ways to see your CJDNS address in Pocketnet Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `pocketcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `pocketcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `pocketcoin-cli help getnodeaddresses` for details.
