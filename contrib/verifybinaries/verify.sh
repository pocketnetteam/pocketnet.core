...
###   pocketcoincore.org and pocketcoin.org and compares them.
...
WORKINGDIR="/tmp/pocketcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://pocketcoincore.org"
HOST2="https://pocketcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="pocketcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pocketcoin-',
...
if [ $? -ne 0 ]; then
   echo "pocketcoin.org failed to provide signature file, but pocketcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "pocketcoin.org and pocketcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pocketcoin Core binary release signing key installed?"
   fi
